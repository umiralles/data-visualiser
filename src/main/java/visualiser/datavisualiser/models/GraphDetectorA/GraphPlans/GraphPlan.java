...
import java.util.stream.Collectors;
 ...
 public abstract class GraphPlan {
    ...
    //  > return a GraphPlan which can then be used to generate the graph
     ...
     static List<List<Attribute>> findMandatoryAndOptionalAttsOrder(List<Attribute> atts,
                                                                           List<AttributeType> mandatories,
                                                                           List<AttributeType> optionals) {
         int attSize = atts.size();
         List<List<Integer>> possMandatories = findPossibleMandatoryAttOptions(mandatories, atts);
         if (possMandatories.size() == 0) {
         ...
         if (numOptionals > 0) {
             ...
             for (List<Integer> possMandOrder : possMandOrders) {
                 List<List<Integer>> possOptionals = findPossibleOptionalAttOptions(optionals, possMandOrder, atts);
    ...
    private static List<List<Integer>> findPossibleMandatoryAttOptions(List<AttributeType> mandatories,
                                                                      List<Attribute> atts) {
    ...
    private static List<List<Integer>> findPossibleOptionalAttOptions(List<AttributeType> optionalTypes,
